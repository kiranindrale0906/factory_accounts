...
class Profit_and_sale_report_model extends BaseModel {
  ...
  public function get_domestic_records($data=array()){
    ...
    $purchase_sales_account_domestic_export_with_vadotar_select = "is_export,
      ...
      purity,
      chitti_id,
      IFNULL(((ac_vouchers.credit_weight*ac_vouchers.factory_purity)/100), 0) as fine,
      IFNULL(((ac_vouchers.credit_weight*ac_vouchers.purity)/100), 0) as gold_fine,
      IFNULL(((ac_vouchers.factory_purity-ac_vouchers.purity)*ac_vouchers.credit_weight/100), 0) as vadotar,
      IFNULL((ac_vouchers.credit_weight), 0) credit_weight";
      $where=array('voucher_type'=>"metal issue voucher","ac_account.group_code"=>"Domestic","ac_account.sub_group_code!="=>"Domestic Labour Account",'chitti_id!=0'=>null);
