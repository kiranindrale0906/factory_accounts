...
class Ledgers extends BaseController {
  ...
  protected function get_datewise_ledger_records() {
    ...
    foreach ($issues as $issue_index => $issue_value) {
      ...
      $ac_voucher_issue_detail=$this->voucher_model->get('metal_receipt_voucher_reference_id,id',array('where'=>array('metal_receipt_voucher_reference_id is not NULL'=>NULL,'id in ('.$issue_value['voucher_id'].')'=>NULL)));
    ...
    if ($this->data['report_type'] == 'Account Ledger'){
      ...
      foreach ($receipts as $receipt_index => $receipt_value) {
        ...
        $ac_voucher_receipt_detail=$this->voucher_model->get('metal_receipt_voucher_reference_id',array('where'=>array('metal_receipt_voucher_reference_id is not NULL'=>NULL,'id in ('.$issue_value['voucher_id'].')'=>NULL)));
