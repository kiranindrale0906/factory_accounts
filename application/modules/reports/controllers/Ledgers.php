...
class Ledgers extends BaseController {
  ...
  protected function get_datewise_ledger_records() {
    ...
    ((factory_purity-purity)*credit_weight/100) as vadotar, (debit_amount - credit_amount) as credit_weight, 0 as `id`,0 as debit_amount,0 as usd_credit_amount,account_id,0 as debit_weight,0 as credit_amount,description,narration,chitti_id as chitti_no,purity,factory_purity,factory_fine';
    ...
    if($this->data['report_type']=='Domestic Labour Ledger'){
     $receipts = $this->voucher_model->get($domestic_export_receipt_issue_select,array('account_name' => 'DOMESTIC LABOUR ACCOUNT'), array(), array('order_by'=>'parent_id, voucher_type asc', 'group_by' => $this->data['group']));
    ...
    $where['ounce_rate != 0 or usd_rate != 0'] = NULL;
      $domestic_export_receipt_issue_select='account_name,date as voucher_date,date_format(date,"%Y-%m-%d") as str_voucher_date,0 as fine, 0 as vadotar, (labour_usd_amount * usd_rate) + (freight_usd_amount * usd_rate) as credit_weight, 0 as `id`,0 as debit_amount,0 as usd_credit_amount,usd_debit_amount,account_id,0 as debit_weight,0 as credit_amount,description,narration,id as chitti_no,purity,factory_purity,factory_fine';

     $receipts = $this->chitti_model->get($domestic_export_receipt_issue_select,$where, array()/*, array('order_by'=>'parent_id, voucher_type asc', 'group_by' => $this->data['group']*/);
