  ...
  protected function get_datewise_ledger_records() {
   ...
   if($this->data['report_type'] == 'Summary Report'){
    ...
    if ($this->data['report_type'] == 'Account Receipt Report'){
      ...
      if ($this->data['site_name'] == 'AR Gold (Apr 2023)'){
        $account_issue_where['account_name'] = 'AR Gold Software (Apr 2023)';
      }elseif ($this->data['site_name'] == 'ARF (Apr 2023)'){
        $account_issue_where['account_name'] = 'ARF Software (Apr 2023)';
      }elseif ($this->data['site_name'] == 'ARC (Apr 2023)'){
        $account_issue_where['account_name'] = 'ARC Software (Apr 2023)';
      }elseif ($this->data['site_name'] == 'AR Gold (Sep 2023)'){
        $account_issue_where['account_name'] = 'AR Gold Software (Sep 2023)';
      }elseif ($this->data['site_name'] == 'ARF (Sep 2023)'){
        $account_issue_where['account_name'] = 'ARF Software (Sep 2023)';
      }elseif ($this->data['site_name'] == 'ARC (Sep 2023)'){
        $account_issue_where['account_name'] = 'ARC Software (Sep 2023)';
      }elseif ($this->data['site_name'] == 'AR Gold (Apr 2024)'){
    ...
    if ($this->data['domestic_export'] == 'Export') {
      ...
      if (str_contains($this->data['site_name'], 'Aug'))
        ...
        elseif ($this->data['site_name'] == 'AR Gold' || $this->data['site_name'] == 'AR Gold (May 2022)' || $this->data['site_name'] == 'AR Gold (Apr 2023)'|| $this->data['site_name'] == 'AR Gold (Apr 2024)'|| $this->data['site_name'] == 'AR Gold (Sep 2023)')
    ...
    if ($this->data['domestic_export'] == 'Domestic') {
        ...
        elseif (   $this->data['site_name'] == 'ARC' 
                || $this->data['site_name'] == 'ARC (Sep 2023)'
                || $this->data['site_name'] == 'ARC (Apr 2024)'
        ...
        elseif (   $this->data['site_name'] == 'AR Gold' 
                || $this->data['site_name'] == 'AR Gold (Sep 2023)'
                || $this->data['site_name'] == 'AR Gold (Apr 2024)'
...
//pd($issues);
    ...
    if ($this->data['report_type'] == 'Account Ledger' || $this->data['report_type'] == 'Purchase Sales Ledger'){
      foreach ($issues as $issue_index => $issue_value) {
        ...
        $ac_voucher_issue_detail=$this->voucher_model->get('metal_receipt_voucher_reference_id,id',array('where'=>array('metal_receipt_voucher_reference_id is not NULL'=>NULL,'id in ('.$voucher_id.')'=>NULL)));
        $metal_receipt_voucher_reference_id=array_column($ac_voucher_issue_detail,'metal_receipt_voucher_reference_id');
        $ac_voucher_issue_credit_weight_details=$this->voucher_model->find('sum(credit_weight) credit_weight,sum(factory_fine) chitti_fine',array('chitti_id'=>$issue_value['chitti_no'],'voucher_type="metal issue voucher"'=>NULL));
    ...
    foreach($this->data['voucher_dates'] as $created_date) {
      ...
      foreach($records[$created_date] as $account_name => $record) {
        if ($record['credit_weight'] !=0 or $record['credit_amount'] != 0 or $record['usd_credit_amount'] != 0){
          ...
          $this->data['day_total'][$record['voucher_date']]['issue']['chitti_credit_weight'] += $record['chitti_credit_weight'];
          $this->data['day_total'][$record['voucher_date']]['issue']['chitti_fine'] += $record['chitti_fine'];
