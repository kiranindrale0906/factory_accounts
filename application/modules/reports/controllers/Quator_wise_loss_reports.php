     ...
     $data['department_names']=array_unique($category_names);
      ...
      if(!empty($data['department_names']) && !empty($this->data['quator_name'])){
        if(isset($_GET['site_name'])&&$_GET['site_name']=='ARC (May 2022)'){
          ...
          $fire_tounch_loss=$this->voucher_model->get('description,site_name,credit_weight as in_weight,purity as in_lot_purity,argold_id as parent_id,0 as out_weight', array('account_name'=>'ARF Loss Account (Apr 2023)','site_name'=>'ARF (Apr 2023)','receipt_type'=>'Ghiss Melting Loss','quator'=>$data['quator']),array());
          foreach ($fire_tounch_loss as $fire_tounch_loss_index => $fire_tounch_loss_value) {
          $data['issue_department_id']=$fire_tounch_loss_value['parent_id'];
          $data['quator']=$this->data['quator_name'];
          $url=API_APR2023_ARF_PATH."issue_and_receipts/loss_report_for_accounts/index";
