...
//pd($category_names);
     $data['department_names']=array_unique($category_names);
      ...
      if(!empty($data['department_names']) && !empty($this->data['quator_name'])){
       if(isset($_GET['site_name'])&&$_GET['site_name']=='AR Gold (Apr 2024)'){
...
*/
        ...
        }elseif(isset($_GET['site_name'])&&$_GET['site_name']=='ARF (Aug 2024)'){
          $ghiss_melting_loss=array();
          $fire_tounch_loss=array();
        ...
        /*foreach ($ghiss_melting_loss as $ghiss_melting_loss_index => $ghiss_melting_loss_value) {
          ...
          $ghiss_melting_loss=$this->voucher_model->get('description,site_name,credit_weight as in_weight,purity as in_lot_purity,argold_id as parent_id,0 as out_weight', array('account_name'=>'ARF Loss Account (Aug 2024)','site_name'=>'ARF (Aug 2024)','receipt_type'=>'Ghiss Melting Loss','quator'=>$data['quator']),array());
           $department_ids = array_column($ghiss_melting_loss, 'parent_id');
          ...
          $fire_tounch_loss=$this->voucher_model->get('description,site_name,credit_weight as in_weight,purity as in_lot_purity,argold_id as parent_id,0 as out_weight', array('account_name'=>'ARF Loss Account (Aug 2024)','site_name'=>'ARF (Aug 2024)','receipt_type'=>'Fire Tounch Loss','quator'=>$data['quator']),array());
         $fire_tounch_loss_department_ids = array_column($fire_tounch_loss, 'parent_id');
