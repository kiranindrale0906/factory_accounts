...
class Chalans extends BaseController {
  ...
  public function _get_view_data() {
    ...
    $this->data['total_of_chitti_details'] = $this->chitti_model->find('sum(taxable_amount) as taxable_amount,sum(weight) as weight,sum(debit_amount) as debit_amount,sum(credit_weight) as credit_weight,sum(cgst_amount) as cgst_amount,sum(sgst_amount) as sgst_amount,sum(rate) as rate', array('chalan_id' => $this->data['record']['id']));
  }
