  ...
  public function _get_view_data() {
    ...
    if($this->data['group_by']==1) {
      if($this->data['item_code']==1){
        $select='sum(fine) as fine,sum(rate) as rate,sum(factory_fine) as factory_fine,sum(credit_weight) as credit_weight,group_concat(narration) as narration,purity,chitti_purity,factory_purity,customer_name,group_concat(distinct(item_code)) as item_code';
