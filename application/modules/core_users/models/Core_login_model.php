...
class  Core_login_model extends BaseModel {
  ...
  public function __construct($data=array()) {
    ...
    $this->load->model('users/users_user_role_model');
  }
  ...
  public function verify_password($encrypted_password) {
    $hashed_password = $this->login_model->find('encrypted_password', array("email_id" => $this->attributes['email_id']), '', array('row_array' => true));
    if(!empty($hashed_password)){
      $verified = (md5($encrypted_password) == $hashed_password['encrypted_password']) ? true : false;
      if(password_verify($encrypted_password, $hashed_password['encrypted_password'])){
        return true;
      }else{
  ...
  public function save($after_save = true){
    if(is_api_request()) {
      $this->formdata['login']['authToken'] = $this->_generate_token();
      parent::update(false);
    } else {
      $this->update(true);
    }
  ...
  public function user_has_role($email_id) {
    $user = $this->login_model->get('id', array("email_id" => $this->attributes['email_id']), '', array('row_array' => true));
