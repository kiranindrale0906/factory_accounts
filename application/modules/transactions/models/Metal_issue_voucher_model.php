...
class Metal_issue_voucher_model extends BaseModel {
  ...
  public function validation_rules($klass='') {
    return array(
      array(
        'field' => 'metal_issue_voucher[voucher_date]', 
        'label' => 'Date',
        'rules' => array('trim', 'required', 
                    array('validate_voucher_date', array($this->metal_issue_voucher_model, 'check_period_exists'))),
        'errors'=>array('validate_voucher_date' => "Please set the period from master.")),
      array(
        'field' => 'metal_issue_voucher[purity]', 
        'label' => 'Purity',
        'rules' => 'trim|required|numeric|less_than_equal_to[100]'),
      array(
        'field' => 'metal_issue_voucher[account_name]', 
        'label' => 'Account Name',
        'rules' => 'trim|required'),
      array(
        'field' => 'metal_issue_voucher[credit_weight]', 
        'label' => 'Credit Weight',
  ...
  public function save_association_data($data, $action) {
    ...
    if ($voucher_number) {
      ...
      $ledger_data['credit_weight'] = $data['credit_weight'];
      $ledger_data['purity'] = $data['purity'];
      $ledger_data['purity_id'] = $data['purity_id'];
      $ledger_data['pure_gold'] = $data['pure_gold_credit'];
